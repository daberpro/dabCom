...
function createNativeComponentFromHTML(Component,component,parent,rawText){

	let text = ``;
	let attr = {};
	const position = Math.round(Math.random() * new Date().getTime()).toString("36");
	...
	if(Component.childNodes instanceof Array && Component.childNodes.length > 0){
		...
		for(let x of Component.childNodes){
			...
			if(x.nodeName !== "#text"){
				...
				createNativeComponentFromHTML(x,component,position,rawText);
...
function transformComponent(rawComponent,res,positionComponent,indexComponent){
	...
	rawComponent = rawComponent.replace(/\$beChild/igm,`parent={parentComponent} componentId={positionComponent}`)
